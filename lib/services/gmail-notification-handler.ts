 ...
 * This is called by the Workspace Connect SDK when a notification arrives
 */
export async function handleGmailPubSubNotification(
  ...
  if (!connection) {
    ...
    throw new Error(`Gmail connection not found for ${emailAddress}`);
  }
...
async function processNewMessage(
  ...
  try {
    ...
    const [existing] = await db
      .select()
      .from(emailLog)
      .where(eq(emailLog.messageId, parsed.id))
      .limit(1);
